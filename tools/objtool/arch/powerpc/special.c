...
int process_exception_entries(struct objtool_file *file)
{
	...
	for (i = 0; i < nr; i++) {
		...
		if (exaddr >= fe_alt_start && exaddr < fe_alt_end) {
...
int process_bug_entries(struct objtool_file *file)
{
	...
	for (i = 0; i < nr; i++) {
		...
		bugaddr = entry_addr + (int32_t)f32_to_cpu(file, bug->bug_addr_disp);

		if (bugaddr >= fe_alt_start && bugaddr < fe_alt_end) {
			printf("Bug entry in alternate section at 0x%lx\n", bugaddr);
			return -1;
		}
	}
	return 0;
